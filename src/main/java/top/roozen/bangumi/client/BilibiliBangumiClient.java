...
public class BilibiliBangumiClient implements IBilibiliBangumiClient {
     ...
     */
    private static final String BILI_API_URL =
        "https://api.bilibili.com/x/space/bangumi/follow/list?type={typeNum}&follow_status={status}&vmid={vmid}&ps={ps}&pn={pn}";
     ...
     * @return Mono响应对象，包含番剧数据的data节点
     */
    @Override
    public Mono<BilibiliApiResponse.DataNode> listBiliBangumiByPage(BilibiliBangumiRequest request) {
        return webClient.get()
            .uri(BILI_API_URL,
                ...
                request.getPn())
            .retrieve()
            .bodyToMono(BilibiliApiResponse.class)
            .map(BilibiliApiResponse::getData);
    }
}
